...
def _patch_boto3_and_env():
    ...
    # Default: no existing order row, no STORE#CONFIG item.
...
TEE_M_UID = "apparel_product_gca_t-shirt_gsc_crewneck_gsi_m"


def _stub_get_item(order_row: dict | None = None, gelato_uids: dict | None = None):
    """Route mock get_item by PK: STORE#CONFIG -> admin UID map, ORDER# -> row.

    The webhook reads both the idempotency row and the admin-set Gelato UID
    config, so a single return_value can't serve both lookups.
    """
    if gelato_uids is None:
        gelato_uids = {"tee-classic/m": TEE_M_UID}

    def side_effect(Key=None, **_kwargs):
        pk = (Key or {}).get("PK", "")
        if pk == "STORE#CONFIG":
            return {"Item": {"PK": pk, "SK": pk, "gelato_uids": gelato_uids}}
        if pk.startswith("ORDER#") and order_row is not None:
            return {"Item": order_row}
        return {}
...
class TestHappyPath:
    def test_checkout_session_completed_writes_submitted_row(self, _patch_boto3_and_env):
        ...
        # Gelato UID from the admin-set STORE#CONFIG item, print file from
        # store_catalog — never from the cart.
        assert kwargs["line_items"] == [{
            "gelato_variant_uid": TEE_M_UID,
            "qty": 2,
...
class TestUnknownSku:
    ...
    def test_sku_without_configured_uid_writes_failed_row(self, _patch_boto3_and_env):
        """Known SKU but no admin-set Gelato UID -> fail-safe, no Gelato call."""
        handler = _patch_boto3_and_env
        _stub_get_item(gelato_uids={})  # nothing configured yet
        ...
        item = mock_table.put_item.call_args.kwargs["Item"]
        assert item["status"] == "failed"
        assert "tee-classic/m" in item["status_error"]
        assert "Gelato UID" in item["status_error"]
