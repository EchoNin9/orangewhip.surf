...
2. **Configure Branches**:
   - **Production**: `main` branch → `orangewhip.surf`
   - **Staging**: `stage` branch → `stage.orangewhip.surf`
...
2. **Testing**: Merge `develop` → `stage` for staging deployment
3. **Production**: Merge `stage` → `main` for production deployment

### Bot Behavior

- The daily bot commits to the `stage` branch at 7:05 AM Pacific Time
- Changes are automatically deployed to the staging site
- Merge `stage` → `main` to deploy bot updates to production
